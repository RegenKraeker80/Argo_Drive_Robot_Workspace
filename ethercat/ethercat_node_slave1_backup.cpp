...
private:
    ...
    void cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
        ...
        target_velocity = static_cast<int>(msg->linear.x * 100); // Geschwindigkeit in Prozent
    ...
    void update() {
        ...
        if (current_velocity < target_velocity) {
            current_velocity = std::min(current_velocity + 10, target_velocity);
        } else if (current_velocity > target_velocity) {
            current_velocity = std::max(current_velocity - 10, target_velocity);
        }
